...
{
    public function getMockedApiReturnListing(): stdClass
    {
        $rawReturnString = <<<EOF
        {
            "data": [
    ...
    public function mockApiReturnSinglePokemon(): stdClass
    {
        $mockedJsonReturnSinGlePokemon =<<<EOF
        {
            "data": {
                "id": "dp3-1",
                "name": "Ampharos",
        ...
        $rawObject = json_decode($mockedJsonReturnSinGlePokemon);

        return $rawObject;
    }

    public function mockApiReturnNonExistent(): stdClass
    {
        $apiMessage =<<<EOF
        {
            "error": {
        ...
        $rawObject = json_decode($apiMessage);

        return $rawObject;
    }

    public function mockApiReturnFragment(string $name = "Ampharos")
    {
        $fragmentString = <<<EOF
        {
            "id": "dp3-1",
        ...
        $fragmentStringWithName = sprintf($fragmentString, $name);

        $rawObject = json_decode($fragmentStringWithName);
